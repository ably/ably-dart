...
2. Update the version in these files:
   ...
   - `README.md` — the `ably: ^OLD_VERSION` line in the Installation section (update to `^NEW_VERSION`)
   - `lib/src/impl/http/constants.dart` - the SDK constants
   - `example/lib/ui/system_details_sliver.dart` - the example app sliver

3. Commit all changed files together with message: `chore: bump version to NEW_VERSION`
...
4. Fetch merged PRs since the last release tag:
   ```
...
5. In `CHANGELOG.md`, insert the following block immediately after the `# Changelog` heading (and its trailing blank line), before the first existing `## [` version entry:
...
6. Commit `CHANGELOG.md` with message: `docs: update CHANGELOG for NEW_VERSION`

...
class SimpleVCDiffDecoder implements VCDiffDecoder {
  ...
  Uint8List decode(Uint8List delta, Uint8List base) {
    ...
    while (offset < delta.length) {
      ...
      if (winIndicator & 0x01 != 0) {
        ...
        sourceSegment =
            Uint8List.sublistView(base, segPos.value, segPos.value + segLen.value);
      ...
      while (instOffset < addrStart) {
        ...
        if (type1 != _VCDiffInstType.noop) {
          ...
          _executeInstruction(
            type1, size1.value, delta, windowOutput,
            sourceSegment, dataOffset, addrOffset, addrStart, delta,
          );
        ...
        if (type2 != _VCDiffInstType.noop) {
          ...
          _executeInstruction(
            type2, size2.value, delta, windowOutput,
            sourceSegment, dataOffset, addrOffset, addrStart, delta,
          );
  ...
  static _IntResult _instructionSize(
    int nibble, Uint8List delta, int instOffset,
  ) {
    ...
    switch (type) {
      ...
      case _VCDiffInstType.copy:
        if (sourceSegment != null) {
          ...
          output.add(Uint8List.sublistView(
            sourceSegment, addr.value, addr.value + size,
          ),);
        }
...
void main() {
  ...
  group('Delta Decoding Integration Tests', () {
    // -------------------------------------------------------------------------
    ...
    test(
      ...
      for (var i = 0; i < 5; i++) {
        ...
        expect(byName[key]!.data, equals(testData[i]),
      ...
      final recoveryChanges = stateChanges.where(
        (sc) =>
            sc.current == ChannelState.attaching &&
            sc.reason?.code == 40018,
      );
      expect(
        recoveryChanges,
        isNotEmpty,
    ...
    test('PC3 - No plugin causes FAILED state', () async {
      ...
      await pollUntil(
        () async =>
            subChannel.state == ChannelState.failed ? true : null,
        timeout: const Duration(seconds: 20),
      );
